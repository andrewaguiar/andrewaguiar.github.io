...
a world like `sapatinhos` (little shoes in portuguese) we want to:
  ...
  - Remove the plural (remove `s`).
  - Remove gender (remove `o`).
  - Remove diminutive (remove `inho`).

This way searching for `sapatinhos` would match terms like `sapato`, `sapatos`, `sapatinho` as well. As you can think
stemming is very language dependent.
...
feminine_reduction if ends_with('a')
augmentative_diminutive_reduction
adverb_reduction
noun_suffix_reduction
verb_suffix_reduction if noun_suffix_reduction_changed_nothing
